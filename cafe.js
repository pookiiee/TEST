 ...
 * Game Engine, Upgrades Shop, Simple Clickable Avatars & Expanded Levels
 */
...
// ==========================================
// 2. أيقونات الشخصيات الكيوت المحدثة
// ==========================================
...
function setupAvatarChoiceElements() {
  ...
  const list = [
    { key: 'bow', icon: ANIMAL_AVATARS.bow },
    { key: 'fox', icon: ANIMAL_AVATARS.fox },
    { key: 'shiba', icon: ANIMAL_AVATARS.shiba },
    { key: 'bear', icon: ANIMAL_AVATARS.bear },
    { key: 'bunny', icon: ANIMAL_AVATARS.bunny },
    { key: 'cat', icon: ANIMAL_AVATARS.cat }
  ];
  ...
  list.forEach((item, index) => {
    ...
    btn.style.cssText = `
      ...
      border-radius: 14px;
      border: 2.5px solid ${index === 0 ? 'var(--pink-main)' : 'var(--pink-subtle)'};
      background: ${index === 0 ? 'var(--pink-subtle)' : '#fff'};
    ...
    btn.addEventListener('click', (e) => {
      ...
      document.querySelectorAll('.avatar-choice').forEach(c => {
        c.style.borderColor = 'var(--pink-subtle)';
      ...
      btn.style.background = 'var(--pink-subtle)';
      
      state.player.avatar = item.icon;
      showToast(`تم اختيار الأيقونة ${item.icon} بنجاح! ✨`);
    });
...
function renderPlayersChips() {
  ...
  state.players.forEach(p => {
    ...
    chip.innerHTML = `<span style="font-size:20px; display:inline-block;">${p.avatar}</span> <span>${p.name}</span> ${p.isHost ? '👑' : ''}`;
    ...
    card.innerHTML = `
      <div style="display:flex; align-items:center; justify-content:space-between;">
        <div style="display:flex; align-items:center; gap:6px;">
          <span style="font-size:18px; display:inline-block;">${order.customerAvatar}</span>
...
function updateStatsDisplay() {
  ...
  if (coinsVal) scoreVal.textContent = state.coins;
